            ...
            for client in clients:
                if client != client_socket:
                    try:
                        ...
                        # Xóa client nếu gửi không thành công
                        clients.remove(client)
                        client.close()
    except Exception as e:
        print(f"Lỗi xử lý client {client_socket.getpeername()}: {e}")
    finally:
        # Xóa client khỏi danh sách và đóng socket khi kết thúc
...
while True:
    ...
    # Tạo SSL context
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)  # Changed from PROTOCOL_TLS to PROTOCOL_TLS_SERVER for server-side
